...
-- see com.renoise.ExampleTool.xrns/main.lua for tools in general
...
local function dynamic_content()
  ...
  my_column_view:add_view(my_text_view)
...
local function dynamic_building_matrix()
  ...
  for octave = 1, NUM_OCTAVES do
    ...
    for note = 1, NUM_NOTES do
      ...
      octave_row:add_view(note_button)
    end

    dialog_content:add_view(octave_row)
  end
...
local function aligners_and_auto_sizing()
...
local function available_backgrounds()
...
local function available_controls()
  ...
  local bitmapview_row = vb:row {
    views = {
      ...
      vb:bitmap {
        ...
        -- enable, to recolor the bitmap to match the GUI theme:
        color = "selected_button_back",
        notifier = function()
...
local function canvas_view()
  -- A canvas view lets you draw and handle mouse events in a completely
  -- customisable way.
  --
  -- Canvas content is cached in a texture and **not** hardware accelerated,
  -- so it's not suitable for animations, but for static content such as
  -- custom backgrounds or bitmap-like views.
  --
  -- In this example we are using a canvas to draw a custom background view
  -- which is updated when the mouse is hovering in the view.

  local vb = renoise.ViewBuilder()

  local CANVAS_WIDTH = 250
  local CANVAS_HEIGHT = 250

  local DIALOG_MARGIN = renoise.ViewBuilder.DEFAULT_DIALOG_MARGIN
  local CONTENT_SPACING = renoise.ViewBuilder.DEFAULT_CONTROL_SPACING

  local canvas_background = vb:canvas {
    id = "canvas_view",
    width = CANVAS_WIDTH,
    height = CANVAS_HEIGHT,
    -- we do not cover, paint on the entire canvas
    -- if we would, set to "plain" to speed up drawing
    mode = "transparent",
    render = function(context)
      -- Build a star path.
      context:move_to(128, 28)
      context:line_to(157, 87)
      context:line_to(223, 97)
      context:line_to(175, 143)
      context:line_to(186, 208)
      context:line_to(128, 178)
      context:line_to(69, 208)
      context:line_to(80, 143)
      context:line_to(32, 97)
      context:line_to(98, 87)
      context:close_path()
      -- Fill the star with yellow color.
      local yellow = { 200, 200, 20, 255 }
      context.fill_color = yellow
      context:fill()
      -- Draw the star with a thick red stroke and rounded points.
      context.line_join = "rounded"
      context.line_width = 12
      local red = { 200, 0, 50, 255 }
      context.stroke_color = red
      context:stroke()
      -- Draw the star again with a dashed thinner orange stroke.
      context:set_line_dash({ 21, 9, 1, 9, 7, 9, 1, 9 })
      context.line_dash_offset = 10.0
      context.line_cap = "circle"
      context.line_width = 6;
      local orange = { 250, 100, 10, 255 }
      context.stroke_color = orange
      context:stroke()
      -- Add a shine layer over the star.
      local white_000 = { 255, 255, 255, 0 }
      local white_200 = { 255, 255, 255, 200 }
      context:set_fill_linear_gradient(64, 0, 192, 256);
      context:add_fill_color_stop(0.30, white_000);
      context:add_fill_color_stop(0.35, white_200);
      context:add_fill_color_stop(0.45, white_200);
      context:add_fill_color_stop(0.50, white_000);
      context.global_composite_operation = "source_atop";
      context:fill_rect(0, 0, context.size.width, context.size.height);
    end,
  }

  local mouse_pos = { x = -1, y = -1 }
  local canvas_overlay = vb:canvas {
    id = "canvas_overlay",
    width = CANVAS_WIDTH,
    height = CANVAS_HEIGHT,
    mouse_events = { "enter", "exit", "move" },
    mouse_handler = function(event)
      if event.type == "exit" or #event.hover_views > 0 then
        mouse_pos = { x = -1, y = -1 }
      else
        mouse_pos = { x = event.position.x, y = event.position.y }
      end
      ---@type renoise.Views.Canvas
      local overlay = vb.views.canvas_overlay
      overlay:update()
    end,
    render = function(context)
      -- Draw shine layer at the cursor position.
      if mouse_pos.x > 0 and mouse_pos.y > 0 then
        context:set_fill_radial_gradient(
          mouse_pos.x, mouse_pos.y, 10, mouse_pos.x + 20, mouse_pos.y + 20, 50)
        context:add_fill_color_stop(0.20, { 255, 255, 255, 80 })
        context:add_fill_color_stop(0.35, { 255, 255, 255, 100 })
        context:add_fill_color_stop(0.45, { 255, 255, 255, 200 })
        context:add_fill_color_stop(0.50, { 255, 255, 255, 0 })
        context:fill_rect(mouse_pos.x - 50, mouse_pos.y - 50, 100, 100)
      end
    end
  }
    ...
    views = {
      vb:stack {
        ...
        views = {
          ...
          vb:link {
            id = "link",
            origin = { 0, CANVAS_HEIGHT },
            text = "HTML5 Canvas Tutorial",
            notifier = function()
              renoise.app():open_url(
  ...
  renoise.app():show_custom_dialog("Canvas View", dialog_content)
end
...
---@field pad_x renoise.Document.ObservableNumber
---@field pad_y renoise.Document.ObservableNumber
...
local function overflow_and_scrollbars()
  local vb = renoise.ViewBuilder()
  ...
  for i = 1, NUMBER_OF_ITEMS_TO_SHOW, 1 do
    content:add_view(
      vb:row {
        ...
        vb:text {
          text = ("This is dummy row #%s"):format(i) }
      })
  end
...
local function handle_mouse_events()
  ...
  local content = vb:stack {
    ...
    mouse_handler = function(event)
      ...
      local mod_flags = 0
      for _, v in pairs(event.modifier_flags) do
        if v then
          mod_flags = mod_flags + 1
        end
      end
      ---@type renoise.Views.Text
...
hover_view (top-most): '%s', mod_flags: '%s']],
      ...
      elseif event.type == "drag" and drag_status.dragging then
        ...
        new_origin.x = math.max(0,
          math.min(CONTENT_WIDTH - BUTTON_WIDTH, new_origin.x))
        new_origin.y = math.max(0,
          math.min(CONTENT_HEIGHT - BUTTON_HEIGHT, new_origin.y))
...
local function handle_key_events()
