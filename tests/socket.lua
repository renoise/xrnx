...
do
  ...
  for _,protocol in pairs{renoise.Socket.PROTOCOL_TCP} do
    ...
    -- get page request
    local message, socket_error = client:receive("*all", receive_timeout)
    ...
    message, socket_error = client:receive(1024, receive_timeout)
    assert(not message and socket_error)
