...
do
  ...
  selected_track.color_blend = 50
  assert(selected_track.color_blend == 50)
  
  local blend = selected_track.color_blend
  assert(blend == 50)
  
  assert_error(function()
    selected_track.color_blend = 101
  end)
  ...
  assert(table.find(available_routings, "Master") ~= nil)
  ...
  assert_error(function()
    master_tracks[1].sample_effects_column_visible = true
  end)
  ...
  seq_tracks[1].sample_effects_column_visible = true
  
  seq_tracks[1]:set_column_is_muted(1, true)
  assert_error(function()
    seq_tracks[1]:set_column_is_muted(0, true)
  end)
  assert_error(function()
    seq_tracks[1]:set_column_is_muted(13, true)
  end)
  
  seq_tracks[1]:set_column_name(1, "Wurst1")
  seq_tracks[1]:set_column_name(2, "Wurst2")
  assert_error(function()
    seq_tracks[1]:set_column_name(0, true)
  end)
  assert_error(function()
    seq_tracks[1]:set_column_name(13, true)
  end)
  
  seq_tracks[1]:swap_note_columns_at(1,2)
  assert(seq_tracks[1]:column_name(1) == "Wurst2")
  assert(seq_tracks[1]:column_name(2) == "Wurst1")
  assert_error(function()
    seq_tracks[1]:swap_note_columns_at(0,1)
  end)
  seq_tracks[1]:swap_effect_columns_at(1,2)
  assert_error(function()
    seq_tracks[1]:swap_effect_columns_at(1,13)
  end)
  
  seq_tracks[1].visible_note_columns = 2
  ...
  assert(song.selected_track_device_index == 1)
  
  song.selected_track_device_index = 0
  assert(song.selected_track_device_index == 0)
  ...
  assert(available_routings[2] == "Master")

  new_group.group_collapsed = true
  new_group.group_collapsed = false

  song:add_track_to_group(3, 2)
  assert(#new_group.members == 2)

  song.tracks[2].name = "t2"

  song:remove_track_from_group(2)
  assert(#new_group.members == 1)
  assert(song.tracks[1].name == "t2")
  assert(rawequal(song.tracks[1].group_parent, nil))

  assert_error(function()
    song:remove_track_from_group(1)
  end)

  song:delete_group_at(3)
